...
struct tickUpdate {
  enum class stage { init, tick0, tick1, finish };
...
public:
  bool update() {
    ...
    const uint64_t now_tick = ::getticks();
...
public:
  ...
  uint64_t getTick() const {
    ...
    uint64_t tick_diff = ::getticks() - mile_stone_tick_;
