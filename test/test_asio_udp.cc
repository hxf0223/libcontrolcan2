...
public:
  UdpClient(const boost::asio::ip::udp::endpoint& listenEndpoint)
      : socket_(io_service_, listenEndpoint)
  ...
  std::size_t receive(const boost::asio::mutable_buffer& buffer, boost::posix_time::time_duration timeout, ec_t& ec) {
...
TEST(asio, udpClient) {
  boost::asio::ip::udp::endpoint const listen_endpoint(boost::asio::ip::address::from_string("0.0.0.0"), kUdpPort);
  UdpClient client(listen_endpoint);

  for (;;) {
    char data[1024];
    boost::system::error_code ec;
    auto recv_num = client.receive(boost::asio::buffer(data), boost::posix_time::milliseconds{100}, ec);
    if (!ec) {
