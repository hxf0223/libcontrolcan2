...
public:
  ...
  void async_send_data() {
    socket_.async_send_to(
        ...
        boost::bind(&udpBroadCastServer::handle_send, this,
...
private:
  ...
  static void handle_receive(const ec_t &ec, std::size_t length, ec_t *out_ec,
...
TEST(asio, udpClient) {
  boost::asio::ip::udp::endpoint listen_endpoint(
      boost::asio::ip::address::from_string("0.0.0.0"), udpPort);
  udpClient c(listen_endpoint);

  for (;;) {
    char data[1024];
    boost::system::error_code ec;
    auto n = c.receive(boost::asio::buffer(data),
