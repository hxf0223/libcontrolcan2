...
public:
  ...
  BlockingReader(boost::asio::serial_port& port, size_t timeout)
      : port_(port)
      , timeout_(timeout)
      , timer_(static_cast<boost::asio::io_context&>(port.get_executor().context())) {}
  ...
  bool readChar(char& val) {
    ...
    (static_cast<boost::asio::io_context&>(port_.get_executor().context())).restart();

    // Asynchronously read 1 character.
    boost::asio::async_read(port_, boost::asio::buffer(&ch_, 1),
    ...
    (static_cast<boost::asio::io_context&>(port_.get_executor().context())).run();
