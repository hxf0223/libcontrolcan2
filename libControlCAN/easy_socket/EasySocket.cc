...
System
  g_system; // 这里实例化了个API初始化对象,使程序在using socket
            // namespace时候就默认的初始化API，结束时自动析构，自动结束API工作 放在这里不和程序其它任何地方的命名冲突
#endif
...
//              of bytes sent
// ====================================================================
int DataSocket::Send(const char *p_buffer, int p_size) const {
  int err;
