...
#ifdef ENABLE_DC_DEBUG
    std::cout << "CanImpDirectCan::VCI_OpenDevice(" << DeviceType << ", " << DeviceInd << ", " << Reserved
              << "): " << err << std::endl;
#endif
...
vciReturnType CanImpDirectCan::VCI_InitCAN(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd,
...
vciReturnType CanImpDirectCan::VCI_ReadErrInfo(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd,
...
vciReturnType CanImpDirectCan::VCI_ReadCANStatus(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd,
...
vciReturnType CanImpDirectCan::VCI_GetReference(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd, DWORD RefType,
                                                PVOID pData) {
  auto const it = _lib_map.find(DeviceType);
  if (it != _lib_map.end()) {
...
vciReturnType CanImpDirectCan::VCI_SetReference(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd, DWORD RefType,
                                                PVOID pData) {
  auto const it = _lib_map.find(DeviceType);
  if (it != _lib_map.end()) {
...
vciReturnType CanImpDirectCan::VCI_Transmit(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd, PVCI_CAN_OBJ pSend,
                                            ULONG Len) {
  auto it = _lib_map.find(DeviceType);
  if (it != _lib_map.end()) {
    // debug output
    /*std::cout << "CanImpDirectCan::VCI_Transmit, devType: " << DeviceType << ", devId: " << DeviceInd << ", canId: "
    << CANInd << std::endl; for ( ULONG i = 0; i < Len && pSend; i++ ) { std::cout.setf(std::ios::hex,
    std::ios::basefield); std::cout << "CanImpDirectCan::VCI_Transmit, ID: " << pSend[i].ID << ", TimeStamp:" <<
    pSend[i].TimeStamp; std::cout << ", TimeFlag: " << (UINT)(pSend[i].TimeFlag) << ", SendType: " <<
    (UINT)(pSend[i].SendType) << ", RemoteFlag: " << (UINT)(pSend[i].RemoteFlag); std::cout << ", ExternFlag: " <<
    (UINT)(pSend[i].ExternFlag) << ", DataLen:" << (UINT)(pSend[i].DataLen);

      std::cout.unsetf(std::ios::hex);
      std::cout << ", Data: " << can::utils::bin2hex_fast(pSend[i].Data, 8) << std::endl;
    }*/
...
ULONG CanImpDirectCan::VCI_Receive(DWORD DeviceType, DWORD DeviceInd, DWORD CANInd, PVCI_CAN_OBJ pReceive, ULONG Len,
                                   INT WaitTime) {
