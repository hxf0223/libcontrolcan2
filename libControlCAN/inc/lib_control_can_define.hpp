...
struct dll_load_dll_type {
  dll_load_dll_type(const std::string &dsoPath) : dll_(dsoPath), path(dsoPath) {
    ...
    // auto &sym_open_dev = dll_.get<DWORD __stdcall(DWORD, DWORD, DWORD)>("VCI_OpenDevice");
    // auto &sym_close_dev = dll_.get<DWORD __stdcall(DWORD, DWORD)>("VCI_CloseDevice");
    ...
    fReadCanStatus =
      import_symbol<DWORD vci_call_conv(DWORD, DWORD, DWORD, PVCI_CAN_STATUS)>(dll_, "VCI_ReadCANStatus");
